...
const movieCard = (movie, i) => {
    return (
        `<div class="flex flex-col gap-3 w-9/12 sm:1/2 md:w-1/3 lg:w-1/4 h-[200px] bg-gray-800 rounded-lg">
                <div
...
function clickHandler(e) {
    ...
    target = e.target;
    if (target) {
        playData = target.attributes['data-play'] || null;
        saveData = target.attributes['data-save'] || null;
        playData ?
            window.open(playData.value, '_blank') :
            saveData ? handleSave(target) : null;
    };
};
