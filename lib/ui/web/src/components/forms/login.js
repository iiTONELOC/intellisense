...
export default function LoginForm() {
    const [checked, setChecked] = useState(localStorage.getItem('jarvis_remember_me') ? true : false)
    const [errorMessage, setErrorMessage] = useState(null)
    const [formState, setFormState] = useState(getRemembered() === null ? { username: null, password: null }
        : getRemembered())
    ...
    const submitFormHandler = async (e) => {
        ...
        if (isFormValidated(formState)) {
            ...
            if (response.status === 200) {
                ...
                if (checked) localStorage.setItem(`jarvis_remember_me`, JSON.stringify(rest));
    ...
    useEffect(() => {
        if (checked === false) localStorage.removeItem('jarvis_remember_me');
    }, [checked]);
    return (
        <>
