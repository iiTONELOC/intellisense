...
from .connection import WebOSWebSocketClient
    ...
    def request(self, uri, params, callback=None, block=False, timeout=60):
        if block:
            ...
            self.client.send_message(
                'request', uri, params, callback=callback)
    ...
    def exec_command(self, cmd, cmd_info):
        def request_func(*args, **kwargs):
            ...
            response_valid = cmd_info.get(
                "validation", lambda p: (True, None))
            return_fn = cmd_info.get('return', lambda x: x)
            block = kwargs.pop('block', True)
            timeout = kwargs.pop('timeout', 60)
            params = process_payload(
                cmd_info.get("payload"), *args, **kwargs)
...
class MediaControl(WebOSControlBase):
    def list_audio_output_sources(self):
        sources = ['tv_speaker', 'external_speaker',
                   'soundbar', 'bt_soundbar', 'tv_external_speaker']
